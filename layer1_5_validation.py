...
def build_ml_dataset(prices, tr_s, tr_e, te_s, te_e, fd, tier_map, min_rows=60):
    ...
    # Pad all arrays to same feature length
    max_feat_tr = max(x.shape[1] for x in X_tr)
    max_feat_te = max(x.shape[0] for x in X_te)
    max_feat = min(max_feat_tr, max_feat_te)

    X_tr_pad = []
    for x in X_tr:
        if x.shape[1] >= max_feat:
            X_tr_pad.append(x[:, :max_feat])
        else:
            pad = np.zeros((x.shape[0], max_feat - x.shape[1]))
            X_tr_pad.append(np.hstack([x, pad]))

    X_te_pad = []
    for x in X_te:
        if x.shape[0] >= max_feat:
            X_te_pad.append(x[:max_feat])
        else:
            x = np.append(x, np.zeros(max_feat - x.shape[0]))
            X_te_pad.append(x)

    X_train = np.vstack(X_tr_pad)
    y_train = np.concatenate(y_tr)
    X_test  = np.array(X_te_pad)
    return X_train, y_train, X_test, te_tickers, te_fwd, te_tiers
...
def validate_granger(universe, stable_edges, tier_map):
    ...
    folds = generate_folds("swing")
    # stable_edges.parquet uses "Source" and "F_stat" (verified from layer 1.55.py)
    cc    = next((c for c in stable_edges.columns if c in ("Source","cause","source")), stable_edges.columns[0])
    fc    = next((c for c in stable_edges.columns if c in ("F_stat","fstat","f_stat")), stable_edges.columns[2])
...
def validate_pairs(universe, spread_history, tier_map):
    log.info("  [6/11] PAIRS")
    folds = generate_folds("long")
    # spread_history.parquet uses "Ticker_A"/"Ticker_B" (verified from layer 1.55.py)
    t1c   = next((c for c in spread_history.columns if c in ("Ticker_A","ticker_a","ticker1")), spread_history.columns[0])
    t2c   = next((c for c in spread_history.columns if c in ("Ticker_B","ticker_b","ticker2")), spread_history.columns[1])
